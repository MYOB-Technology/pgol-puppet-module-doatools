...
describe 'PuppetX::IntechWIFI::DeclareEnvironmentResources::AutoScalerHelpers' do
  let(:helpers) { PuppetX::IntechWIFI::DeclareEnvironmentResources::LoadBalancerHelper }
  ...
  describe "#service_have_loadbalanced_ports?" do
    it "returns false when the service is not defined." do
      expect(helpers.service_have_loadbalanced_ports?(services_1, 'service_0')).to eq(false)
    end

    it "returns false when the service does not have loadbalanced_ports defined." do
      expect(helpers.service_have_loadbalanced_ports?(services_1, 'service_2')).to eq(false)
    end

    it "returns false when the service loadbalanced_ports are empty" do
      expect(helpers.service_have_loadbalanced_ports?(services_1, 'service_3')).to eq(false)
    end

    it "returns true when the service has some load balanced ports defined" do
      expect(helpers.service_have_loadbalanced_ports?(services_1, 'service_1')).to eq(true)
    end
  ...
  describe "#get_role_names_with_load_balancers" do
    it "returns an empty list if there are no roles" do
      expect(helpers.get_role_names_with_load_balancers(roles_0, services_1)).to eq([])
    end

    it "returns an empty list if the role has no services" do
      expect(helpers.get_role_names_with_load_balancers(roles_1, services_1)).to eq([])
    end

    it "returns an empty list if all roles have no services" do
      expect(helpers.get_role_names_with_load_balancers(roles_2, services_1)).to eq([])
    end

    it "returns an empty list if all roles have only have services that do not have load balanced ports" do
      expect(helpers.get_role_names_with_load_balancers(roles_3, services_3)).to eq([])
    end

    it "returns a list of roles containing only the roles that have load balanced ports" do
      expect(helpers.get_role_names_with_load_balancers(roles_3, services_2)).to eq(['role_name_1', 'role_name_2', 'role_name_4'])
    end

  end

  describe "#generate_services_with_loadbalanced_ports_by_role" do
    it "returns an empty hash if there are no roles" do
      expect(helpers.generate_services_with_loadbalanced_ports_by_role(roles_0, services_1)).to eq({})
    end

    it "returns an empty hash if the roles have no services" do
      expect(helpers.generate_services_with_loadbalanced_ports_by_role(roles_1, services_1)).to eq({})
    end

    it "returns an empty hash if all roles have no services" do
      expect(helpers.generate_services_with_loadbalanced_ports_by_role(roles_2, services_1)).to eq({})
    end

    it "returns an empty hash if all roles have only have services that do not have load balanced ports" do
      expect(helpers.generate_services_with_loadbalanced_ports_by_role(roles_3, services_3)).to eq({})
    end

    it "returns a hash that enables lookups by role to a list of services that have load balanced ports" do
      expect(
          helpers.generate_services_with_loadbalanced_ports_by_role(roles_3, services_2)
      ).to eq({
